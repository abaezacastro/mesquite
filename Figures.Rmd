...
image(predictors_past[[2]],zlim=c(-17,30),  col = rev(RColorBrewer::brewer.pal(9, "RdYlBu")),  xlab = 'Longitude', ylab = 'Latitude', main = 'd.  Min. daily Temp. [1900-1950]',breaks=c(-17,-5,0,5,10,15,20,25,30,100))
plot(USA_map,add=T)

image(mask(predictors_present[[2]],USA_map),zlim=c(-17,30),  col = rev(RColorBrewer::brewer.pal(9, "RdYlBu")),  xlab = 'Longitude', ylab = 'Latitude', main = 'e. [1951-2000]',breaks=c(-17,-5,0,5,10,15,20,25,30,100))
plot(USA_map,add=T)

image(mask(predictors_future[[2]],USA_map),zlim=c(-17,30),  col = rev(RColorBrewer::brewer.pal(9, "RdYlBu")),  xlab = 'Longitude', ylab = 'Latitude', main = 'f.  [2080-2010]',breaks=c(-17,-5,0,5,10,15,20,25,30,100))
plot(USA_map,add=T)


#precipitation
image(predictors_past[[3]],zlim=c(0,16),  col = RColorBrewer::brewer.pal(9, "YlGn"),  xlab = 'Longitude', ylab = 'Latitude', main = 'g. Ave. daily Rainfall [1900-1950]',breaks=c(0,1,2,3,4,5,6,7,20,100))
plot(USA_map,add=T)

image(mask(predictors_present[[3]],USA_map),zlim=c(0,16),  col = RColorBrewer::brewer.pal(9, "YlGn"),  xlab = 'Longitude', ylab = 'Latitude', main = 'h.  [1950-2000]',breaks=c(0,1,2,3,4,5,6,7,20,100))
plot(USA_map,add=T)

image(mask(predictors_future[[3]],USA_map),zlim=c(0,16),  col = RColorBrewer::brewer.pal(9, "YlGn"),  xlab = 'Longitude', ylab = 'Latitude', main = 'i. 2080-2100',breaks=c(0,1,2,3,4,5,6,7,20,100))
plot(USA_map,add=T)
...
png(filename = "output_maps/Figure3.png",width = 60,height = 30,units = "cm",res = 200)
...
DiffP2000_T50= (maps_present[[1]]>0.5) - (maps_past[[1]]>0.5) #P2000_F - T50   #total current expansion


#Diff_T50T2100=(T2100>0)-T50 #future expansion due to climate
Diff_T2100P2000=(maps_past[[3]]>0.5) - (maps_present[[1]]>0.5) #(T2100_F>0)-P2000_F #future expansion due to climate


DiffP2100_T2100=(maps_present[[2]]>0.5) - (maps_past[[3]]>0.5)#mask((P2100_F-(T2100_F>0)),USA_map) #future expansion due to LU #Note: use mask() function ro remove areas outside USA

#DiffP2100_T50=P2100 - T50 #Total expansion due to climate and LU

DiffP2100_P2000=(maps_present[[2]]>0.5) - (maps_present[[1]]>0.5)#P2100_F - P2000_F #Total expansion due to climate and LU



png(filename = "output_maps/figure4.png",width = 60,height = 30,units = "cm",res = 200)
par(mfrow=c(2,3),cex=1.2)
